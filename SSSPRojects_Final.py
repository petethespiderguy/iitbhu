...
# try:
    ...
    df = pd.read_csv ('./excelfiles/{}'.format(mapping[table_name]),converters=columns_type)[:100].fillna(value=np.nan)
    ...
    try:
        ...
        # df1['Creation Date'] = pd.to_datetime(df1['Creation Date']).dt.date
        # df1['Deleted on'] = pd.to_datetime(df1['Deleted on']).dt.date
        # df1['Deployment date'] = pd.to_datetime(df1['Deployment date']).dt.date
        # df1['Modified on'] = pd.to_datetime(df1['Modified on']).dt.date
        # df1['Opening date'] = pd.to_datetime(df1['Opening date']).dt.date
        ...
        cursor.execute("ALTER TABLE dbo.{} ALTER COLUMN [Deleted on] date".format(table_name))
        cursor.execute("ALTER TABLE dbo.{} ALTER COLUMN [Deployment date] date".format(table_name))
        cursor.execute("ALTER TABLE dbo.{} ALTER COLUMN [Modified on] date".format(table_name))
        cursor.execute("ALTER TABLE dbo.{} ALTER COLUMN [Opening date] date".format(table_name))
        cursor.execute("TRUNCATE TABLE dbo.{}".format(table_name)) # Truncating the SQL table.
        if table_type:
            print ("HERE!!!")
            # for n,i in enumerate(params):
            #     print (n)
            #     ss = "INSERT INTO dbo.{} VALUES (?".format(table_name)+ "".join(",?"*(no_sheet_col-1)) + ",GETDATE())"
            #     print (ss.format(i))
            #     exit()
